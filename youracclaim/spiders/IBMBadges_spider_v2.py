...
class IBMBadges_spider(scrapy.Spider):  
    ...
    def parse(self, response):
    	...
    	badge_list = sel.xpath('//a[@class="c-grid-item c-grid-item--stack-lt-sm cr-public-organization-badge-template-grid-item"]')
        
    	for badge_info in badge_list:
    		item = YouracclaimItem()
    		item["title"] = badge_info.xpath('@title').extract()[0]
    		item["link"] = "https://www.youracclaim.com" + badge_info.xpath('@href').extract()[0]
    		item["image"] = badge_info.xpath('div/img/@src').extract()[0]
    		#print("title -> " + item["title"])
    		yield scrapy.Request(item['link'], meta={'item': item}, callback=self.detail_parse)
            #    yield item
    ...
    def detail_parse(self, response):
        ...
        item["description"] = response.xpath('//p[@class="cr-badges-full-badge__description"]/text()').extract()[0].replace("\n", "")
        item["skills"] = response.xpath('//p[@class="cr-badges-full-badge__description"]/text()').extract()[0].replace("\n", "")
